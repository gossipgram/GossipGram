...
//     console.log("allUserFollowing",allUserFollowing)
...
//     };
//     useEffect(() => {
//         // Your code here to handle any necessary updates
//     }, [userData]);
...
//                         const isFollowing = allUserFollowing.includes(following.following._id);
...
//                                         onClick={() => followButtonHanler(isFollowing , following.following._id)}
//                                     >
...
const FollowingModal = ({ followerDetails, following, changeIsFollowingModalOpen}) => {
    const token = localStorage.getItem("token").split('"')[1];
                        ...
                        return (
                            <div key={index} className="flex items-center border-b w-full p-2 border-yellow-500 justify-between">
                                ...
                                {isCurrentUser ? (
                                    ...
                                    <button
                                        className={`ml-10 bg-${isFollowing ? 'yellow-100 px-4' : 'blue-100 px-6' } text-richblack-900 rounded-xl font-medium py-[8px] mt-2 hover:bg-${isFollowing ? 'yellow' : 'blue'}-200`}
                                        onClick={() => followButtonHandler(isFollowing , following.following._id)}
                                    >
                                        {isFollowing ? "Following" : "Follow"}
                                    </button>
                                )}
