...
const PostGrid = ({ userId, searchedUserId, matchingUsers }) => {
    ...
    useEffect(() => {
        if (searchedUser) {
            ...
            const filteredPosts = searchedUser?.posts.filter(post => post.mediaUrl.includes("video") || post.mediaUrl.includes("image"));
            ...
            {allUserPost.length === 0 ? (
                ...
                <div className="grid grid-cols-3 gap-1 transition-all duration-700">
                    {allUserPost.map(post => (
                        <div key={post._id} className="w-52 h-52 relative" onMouseEnter={() => handleVideoHover(post)} onMouseLeave={() => setHoveredPost(null)}>
